...
Write-Host ""
Write-Host "------------------------------------------" -ForegroundColor Cyan
Write-Host "4. 检查 Skill 虚拟环境" -ForegroundColor Cyan
Write-Host "------------------------------------------" -ForegroundColor Cyan
...
if (Test-Path $venvPython) {
    Check-Condition $true "Skill 虚拟环境存在"
    
    $libs = @("requests", "urllib3", "yaml", "keyring", "cryptography")
    foreach ($lib in $libs) {
        $checkCmd = "import $lib; print('OK')"
        $result = & $venvPython -c $checkCmd 2>&1
        if ($result -eq "OK") {
            Check-Condition $true "$lib 库已安装 (venv)"
        } else {
            Check-Condition $false "$lib 库已安装 (venv)"
        }
    }
} else {
    Check-Condition $false "Skill 虚拟环境存在"
    Write-Host "  提示: 请运行 .\scripts\setup.ps1 创建虚拟环境" -ForegroundColor Yellow
}
...
if ($checksFailed -eq 0) {
    Write-Host "[OK] 所有检查通过！部署验证成功。" -ForegroundColor Green
    Write-Host ""
    Write-Host "您现在可以在 Trae IDE 中使用 ZTools 了。"
    Write-Host "首次使用时会提示登录禅道。"
} else {
    Write-Host "[WARN] 部分检查未通过，请查看上面的详细信息。" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "建议:"
