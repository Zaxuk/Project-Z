...
try {
    ...
    if ($versionMatch) {
        ...
        if ($major -gt 3 -or ($major -eq 3 -and $minor -ge 13)) {
            Write-Host "[OK] Python 版本符合要求 (>= 3.13)" -ForegroundColor Green
        } else {
            Write-Host "[FAIL] Python 版本过低，需要 3.13 或更高版本" -ForegroundColor Red
            exit 1
        }
    } else {
        Write-Host "[FAIL] 无法解析 Python 版本" -ForegroundColor Red
        exit 1
    }
} catch {
    Write-Host "[FAIL] Python 未安装或不在 PATH 中" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "[FAIL] pip 未安装，请先安装 pip" -ForegroundColor Red
    exit 1
}
...
if (Test-Path $skillDir) {
    ...
    Write-Host "[OK] zentao-helper 依赖安装完成" -ForegroundColor Green
} else {
    Write-Host "[WARN] zentao-helper 目录不存在，跳过" -ForegroundColor Yellow
}
...
if (Test-Path $configFile) {
    Write-Host "[OK] 配置文件已存在: $configFile" -ForegroundColor Green
    ...
    Write-Host "[FAIL] 配置文件不存在: $configFile" -ForegroundColor Red
