...
* **自我审查**：在生成代码后，请自问："这段代码是否符合 `AGENTS.md` 中的错误处理规范？"如果不符合，请自动修正。
* **小步提交**：不要一次性生成 500 行以上的代码。采用"分析 -> 方案确认 -> 实现 -> 验证"的循环。
* **代码提交授权**：任何代码调整（包括但不限于修改、新增、删除文件，以及提交并推送代码到远程仓库），必须等待用户明确发出指令要求后方可执行。严禁自主提交并推送代码。
* **问题分析与验证**：在进行代码修改前，必须先充分了解问题的具体情况，包括但不限于数据状态、业务逻辑、配置设置等。对于复杂问题，应先进行验证步骤，确认问题的根本原因后再提出解决方案。
* **针对性解决方案**：提出的解决方案必须针对具体问题，避免采取过于宽泛的修改（如禁用过滤逻辑、查询所有数据等），确保解决方案既解决问题又不会引入性能问题或其他副作用。
* **避免主观臆断**：严禁在没有充分了解问题具体情况的前提下，主观臆断地调整代码逻辑。当信息不足时，应主动向用户请求协助，获取必要的上下文信息。
