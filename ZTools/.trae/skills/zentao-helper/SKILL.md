...
- 任务拆解：将一个任务拆解为多个子任务
  ...
  - 自动更新需求标题
  - 支持设置任务时长和截至时间
  - **任务时长智能计算**：根据需求等级自动计算默认时长
  - **B等级必填**：B等级需求必须输入任务时长
- 任务分配：将任务分配给指定用户
- **中文界面**：提供 zentao-tools.py 交互式中文界面
  - 集成所有功能的统一菜单
  - 支持双击运行（zentao-tools.bat）
...
# 从需求创建任务（非交互模式）
python skill.py 拆解需求#11754 --grade=A+ --priority=非紧急 --online-time=下下周周一 --assigned-to=chenkeyu --hours=8 --deadline=本周周五

# 简写形式
python skill.py 拆解需求#11754 -g A+ -o 下下周周一 -a chenkeyu
...
| `--priority` | `-p` | 需求优先级 | 非紧急, 紧急 |
| `--online-time` | `-o` | 需求上线时间 | 下周周一, 下周周四, 下下周周一, 下下周周四, YYMMDD格式 |
| `--assigned-to` | `-a` | 任务执行人 | 用户名 |
| `--hours` | | 任务时长 | 数字（小时） |
| `--deadline` | `-d` | 任务截至时间 | 本周周五, 下周周五 |

### 从需求创建任务流程
当执行**拆解需求**时，系统会引导您完成以下步骤：

1. **需求等级选择**：A-/A/A+/A++/B，默认A+
2. **需求优先级选择**（是否紧急）：否/是，默认否
3. **需求上线时间选择**：
   - 下周周一
   - 下周周四
   - 下下周周一 (默认)
   - 下下周周四
   - 自定义日期（格式：YYMMDD，如260331）
   - **日期显示**：同时显示中文描述和 YYMMDD 格式日期
4. **任务执行人输入**：默认为 zhuxu
5. **任务时长输入**：
   - **智能计算**：根据需求等级自动计算默认时长
   - A-: 1小时, A: 2小时, A+: 4小时, A++: 8小时
   - **B等级必填**：B等级需求必须输入任务时长
6. **任务截至时间选择**：本周周五/下周周五，默认本周周五
7. **最终确认**：显示所有信息（包含需求标题和任务标题预览），直接回车或输入y确认，输入n取消。确认后先更新需求标题，再自动创建任务

**注意**：
- 任务名称就是生成的任务标题（格式：【研发】【等级】【标签】YYMMDD 原标题），不需要额外输入
- 需求标题中的【紧急】标签会根据用户选择的"是否紧急"自动添加或移除

### 标题格式
- **需求标题**：【等级】【标签1】【标签2】【标签N】YYMMDD 剩余原标题（上线时间无方括号，空格分隔）
- **任务标题**：【研发】【等级】【标签1】【标签2】【标签N】YYMMDD 剩余原标题
...
zentao:
  ...
  task_creation:
    ...
    default_project_name: null             # 默认关联项目名称（支持部分匹配）
                                           # 例如: "都江堰" 可匹配 "都江堰项目"
                                           # 设为 null 则每次让用户选择
